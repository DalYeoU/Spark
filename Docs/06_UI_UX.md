...
Interaction Prompt는 대상 액터 위치에 부착되어 표시되며,
상호작용 가능한 상황에서만 나타난다.
...
| 요소 | 표시 조건 | 표시 시간 |
|------|-----------|-----------|
| Interaction Prompt (대상 부착) | 상호작용 가능 | 조건 유지 중 |
...
Prompt는 화면에 고정된 HUD 요소가 아니라,
상호작용 대상 액터에 부착되는 다이제틱(Diegetic) UI로 표시한다.

대상과 텍스트를 시각적으로 분리하지 않음으로써,
어떤 오브젝트에 대한 안내인지 즉시 알 수 있게 한다.
...
상호작용 키는 E로 고정이므로, `[ E ]` 키캡 전체를 하나의 이미지 에셋으로 제작해 재사용한다.
E 자체를 TextBlock으로 구현하지 않으며, 연결/분리/활성화처럼 상황에 따라 바뀌는 행동 텍스트만 TextBlock으로 처리한다.
즉 위젯 구조는 `[E 아이콘 이미지] + [행동 TextBlock]`으로 구성한다.

**라인 스타일(가이드 포인트형)**: 대상 지점에서 위로 뻗는 얇은 수직 라인 + 라인 끝(대상 쪽)에 작은 다이아몬드 포인트 마커, 라인 위쪽 끝에 `[E 아이콘] + 행동 텍스트` 박스를 배치한다.
체크포인트 UI와 동일한 색 톤(Primary #FFFFFF, Sub #9CA3AF, Accent #4FA3FF, Dark BG #000000)을 사용한다.
추후 대상별로 라인을 L자형(세로+가로 꺾임으로 대상과의 연결점을 더 명확히 함)으로 교체할 수 있도록, 라인/포인트 요소를 별도 이미지로 분리해 구성한다.

---

## Diegetic Attachment

Interaction Prompt는 `WidgetComponent`를 통해 대상 액터(또는 지정된 소켓)에 부착한다.

- Unlit 렌더링을 사용하여 어두운 환경에서도 텍스트 시인성을 유지한다.
- 항상 카메라를 향하도록 빌보드 처리한다.
- 거리에 따라 크기를 일정 범위 내에서 보정하여 너무 작거나 크게 보이지 않도록 한다.
- 다른 오브젝트에 가려질 경우, 완전히 숨기거나 투과 표시 여부를 프로젝트 기준에 맞게 결정한다.
- 대상이 바뀔 때마다 매번 새로 생성하지 않고 단일 위젯 인스턴스를 재부착하여 사용한다.
