...
Rubber 구간은 스파크가 발생하지 않아 이동 중 시야 확보가 가장 어려운 구간이다. Cable은 강한 스파크 반응으로 이 공백을 메우는 대체 조명 역할을 겸한다 — 퍼즐(전력 복구) 기능과 별개로, Rubber 구간 근처에 배치해 플레이어가 어둠 속에서 방향을 잃지 않도록 돕는 레벨 디자인 장치로 활용한다.
...
-   상호작용 대상 탐지 및 Prompt 표시
-   장치 활성화, 문 열기
-   **케이블 연결**: 방전된 케이블을 찾아 연결하면 전력이 복구되어 문/장치가 열린다. Cable Spark(강한 스파크) 반응과 연결되는 상호작용 오브젝트이며, 시설 전력을 복구해가는 메인 스토리(5장)와도 맞물린다.
